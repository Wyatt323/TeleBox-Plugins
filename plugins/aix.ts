...
const buildPromptComparisonCaption = (
  originalPrompt: string,
  optimizedPrompt?: string,
): string => {
  const original = (originalPrompt || "").trim();
  const optimized = (optimizedPrompt || "").trim();
  if (!optimized || optimized === original) return original || optimized;
  return `原提示词：\n${original}\n\n优化提示词：\n${optimized}`.trim();
};
...
const buildGeneratedPromptListText = (_title: string, prompts: string[]): string =>
  prompts
    .map(
      (prompt, index) =>
...
const runPromptOptimizationWithTimeout = async (
  aiService: AIService,
  request: string,
  ...
  const timeoutId = setTimeout(() => {
    if (!token.aborted) token.abort("请求超时");
  }, timeoutMs);
  try {
    return await aiService.callAI(request, imageParts, token);
  } finally {
...
const isTimeoutLikeErrorText = (text: string): boolean =>
  /请求超时|timeout|timed out|ETIMEDOUT|ECONNABORTED|524|504|Gateway Timeout/i.test(text);

const isSafetyLikeErrorText = (text: string): boolean =>
  /违规|敏感|安全|风控|审核|拦截|不合规|内容政策|policy|safety|blocked|violation|moderation|not allowed|content_filter|ResponsibleAIPolicyViolation/i.test(text);
...
const resolveMergedImageParts = async (
  ...
  for (let i = 0; i < inputItems.length; i++) {
    ...
    const buf = await sharp(inputItems[i].data)
      .resize(cell, cell, { fit: "inside", withoutEnlargement: true })
      .flatten({ background: "white" })
      .extend({ top: 0, bottom: 0, left: 0, right: 0, background: "white" })
      .png()
      .toBuffer();
    const meta = await sharp(buf).metadata();
    const imgW = meta.width || cell;
    const imgH = meta.height || cell;
...
class MessageUtils {
  ...
  private async sendMedia<T extends AIImage | AIVideo>(
    ...
    if (mediaItems.length > 1 && options.previewEnabled) {
      ...
      try {
        ...
        console.log("[aix] album send failed, fallback to sequential send:", error);
      } finally {
...
class ConfigManager {
  ...
  private ensureDefaults(): void {
    ...
    if (!cfg.modelPresets || typeof cfg.modelPresets !== "object") {
      cfg.modelPresets = {};
    } else {
      for (const [name, preset] of Object.entries(cfg.modelPresets)) {
        if (
          !preset ||
...
class ConfigFeature extends BaseFeatureHandler {
  ...
  private async deleteConfig(
    ...
    await configManager.updateConfig((cfg) => {
      ...
      if (cfg.currentSearchTag === delTag) {
        cfg.currentSearchTag = "";
        cfg.currentSearchModel = "";
      }
  ...
  async execute(
    ...
    if (mode === "preset") {
      ...
      if (!action || action === "list") {
        const entries = Object.entries(config.modelPresets || {});
        const lines = entries.length
          ? entries.map(([name, preset]) =>
              `• <code>${name}</code> → 配置 <code>${preset.tag}</code> / 模型 <code>${preset.model}</code>`,
            ).join("\n")
          : "暂无模型预设";
        await this.editMessage(
          msg,
          `🗂️ <b>模型预设:</b>\n\n${lines}\n\n` +
            `添加：<code>.aix model preset add 配置名 提供商 模型</code>\n` +
            `删除：<code>.aix model preset del 配置名</code>`,
        );
        return;
      }

      if (action === "add" || action === "set") {
        requireUser(args.length >= 6, "用法：.aix model preset add 配置名 提供商 模型");
        const presetName = args[3]?.trim();
        const tag = args[4]?.trim();
        const model = args.slice(5).join(" ").trim();
        requireUser(!!presetName && !/\s/.test(presetName), "配置名不能包含空格");
        requireUser(!!config.configs[tag], `提供商配置标签 "${tag}" 不存在`);
        requireUser(!!model, "模型不能为空");
        await configManager.updateConfig((cfg) => {
          cfg.modelPresets[presetName] = { tag, model };
        });
        await this.editMessage(
          msg,
          `✅ 模型预设已保存：<code>${presetName}</code>\n` +
            `🏷️ 提供商：<code>${tag}</code>\n🧠 模型：<code>${model}</code>`,
        );
        return;
      }
      ...
      if (action === "del" || action === "delete" || action === "remove") {
        const presetName = args[3]?.trim();
        requireUser(!!presetName, "用法：.aix model preset del 配置名");
        requireUser(!!config.modelPresets[presetName], `模型预设 "${presetName}" 不存在`);
        await configManager.updateConfig((cfg) => {
          delete cfg.modelPresets[presetName];
        });
        await this.editMessage(msg, `✅ 已删除模型预设：<code>${presetName}</code>`);
        return;
      }
    ...
    requireUser(args.length >= 3, "参数不足");

    let model: string;
    let tag: string;
    let presetName = "";
    if (args.length === 3) {
      presetName = args[2]?.trim();
      const preset = config.modelPresets?.[presetName];
      requireUser(
        !!preset,
        `模型预设 "${presetName}" 不存在；请先用 .aix model preset add 配置名 提供商 模型 添加`,
      );
      tag = preset.tag;
      model = preset.model;
    } else {
      model = args[args.length - 1];
      tag = args.slice(2, -1).join(" ").trim();
    }
    ...
    await this.editMessage(
      msg,
      `✅ ${modeLabel} 已切换到:\n\n` +
        `${presetName ? `🗂️ 预设: <code>${presetName}</code>\n` : ""}` +
        `🏷️ 配置: <code>${tag}</code>\n🧠 模型: <code>${model}</code>`,
    );
  }
}
...
class TimeoutFeature extends BaseFeatureHandler {
  ...
  async execute(
    ...
    requireUser(
      !isNaN(timeout) && timeout >= 1 && timeout <= 1800,
      "超时时间必须在 1 到 1800 秒之间",
    );
...
class QuestionFeature extends BaseFeatureHandler {
  ...
  cancelCurrentOperation(reason: string = "操作已取消"): void {
  ...
  async handleQuestion(
    ...
    if (
      ...
      throw new UserError(
        `请先配置 API 并设置模型\n使用 ${prefixes[0]}aix config add <tag> <url> <key> [type] 和 ${prefixes[0]}aix model chat <tag> <model-path>`,
      );
    }
...
class SearchFeature extends BaseFeatureHandler {
  ...
  async execute(
    ...
    if (
      ...
      throw new UserError(
        `请先配置 API 并设置模型\n使用 ${prefixes[0]}aix config add <tag> <url> <key> [type] 和 ${prefixes[0]}ai model search <tag> <model-path>`,
      );
    }
...
const normalizeSingleImagePrompt = (prompt: string, index: number, total: number): string => {
  let p = prompt.trim();
  p = p
...
只生成一张独立图片，不要拼图，不要九宫格，不要把多张图合在一张里。这是第 ${index + 1}/${total} 张，构图/表情/动作要和其它张有区别。`;
  } else {
    p += `

只生成一张独立图片，不要拼图，不要九宫格。`;
  }
  return p;
};

const isTransientImageError = (error: any): boolean => {
  const status = error?.response?.status;
  const msg = String(error?.message || error?.response?.data?.error?.message || error || "");
  return status === 429 || status === 500 || status === 502 || status === 503 || status === 504 || status === 524 || /timeout|ECONNRESET|ETIMEDOUT|socket|502|503|504|524|429/i.test(msg);
};

const describeImageError = (error: any): string => {
  const status = error?.response?.status;
  const msg = error?.response?.data?.error?.message || error?.response?.data?.message || error?.message || String(error || "未知错误");
  return status ? `${status} ${msg}` : msg;
};


const parseChineseCount = (text: string): number | null => {
  const map: Record<string, number> = {
    一: 1, 二: 2, 两: 2, 俩: 2, 三: 3, 四: 4, 五: 5,
    六: 6, 七: 7, 八: 8, 九: 9, 十: 10,
  };
  const m = text.match(/([一二两俩三四五六七八九十]|十[一二三四五六七八九]|[一二两俩三四五六七八九]十[一二三四五六七八九]?)(?:张|幅|个|套|组)/);
  if (!m) return null;
  const s = m[1];
...
const parseMediaCountRequest = (args: string[], startIndex: number): { prompt: string; count: number } => {
  ...
  for (let i = 0; i < rawTokens.length; i++) {
    ...
    if ((low === "--n" || low === "-n" || low === "--count") && rawTokens[i + 1]) {
      const n = Number.parseInt(rawTokens[i + 1], 10);
...
class ImageFeature extends BaseFeatureHandler {
  ...
  private consumeAvatarLockArg(args: string[]): { args: string[]; forceAvatar: boolean } {
  ...
  private detectSourceLock(args: string[], forceAvatar: boolean): "avatar" | "sticker" | "image" | null {
    if (forceAvatar) return "avatar";
    const tokens = args.slice(1).filter(Boolean);
    const first = tokens[0]?.toLowerCase();
    if (first === "a") return "avatar";
    if (["avatar", "头像", "頭像"].includes(first || "")) return "avatar";
    if (["sticker", "贴纸", "貼紙"].includes(first || "")) return "sticker";
    if (["image", "img", "图片", "圖片"].includes(first || "")) return "image";
    return null;
  }
  ...
  private parseImageRequest(args: string[]): { prompt: string; count: number } {
    ...
    for (let i = 0; i < rawTokens.length; i++) {
      ...
      if (kept.length === 0) {
        const m = low.match(/^(?:n=|count=|x)?(\d+)(?:张|幅|个)?$/);
        if (m) {
          const n = Number.parseInt(m[1], 10);
          if (Number.isFinite(n) && n > 0) {
            count = n;
  ...
  async execute(
    ...
    // 图片提示词优化总开关：.aix image optimize on/off
    // on=先用聊天模型优化提示词再生图；off=直接用原提示词生图。
    if (subCommand === "optimize" && (args[2]?.toLowerCase() === "on" || args[2]?.toLowerCase() === "off") && args.length <= 3) {
      const enabled = args[2]?.toLowerCase() === "on";
      await configManager.updateConfig((cfg) => {
        cfg.promptOptimize = enabled;
      });
      await this.editMessage(
        msg,
        enabled ? "✅ 图片提示词优化已开启" : "✅ 图片提示词优化已关闭",
      );
      return;
    }
    if (subCommand === "optimize" && ["short", "medium", "long"].includes(args[2]?.toLowerCase() || "")) {
      const mode = normalizePromptLengthMode(args[2]);
      await configManager.updateConfig((cfg) => {
        cfg.promptLength = mode;
      });
      await this.editMessage(msg, `✅ 图片优化提示词长度已设为 ${mode}（${IMAGE_PROMPT_LENGTH_LABELS[mode]}）`);
      return;
    }
    ...
    const avatarLock = this.consumeAvatarLockArg(args);
    args = avatarLock.args;
    const forceAvatar = avatarLock.forceAvatar;
    const sourceLock = this.detectSourceLock(args, forceAvatar);
    const imageRequest = this.parseImageRequest(args);
    const optimizeParsed = extractPromptOptimizeFlag(imageRequest.prompt);
    const promptInput = optimizeParsed.prompt;
    ...
    const imageCount = imageRequest.count;
    const wantsAvatar = sourceLock === "avatar";
    
    let replyImageParts = await getMessageImageParts(replyMsg);
    const messageImageParts = await getMessageImageParts(msg);
    let imageParts = [...replyImageParts, ...messageImageParts];
    const originalReplyImageParts = [...replyImageParts];
    const originalImageParts = [...imageParts];


    if (sourceLock === "avatar") {
      imageParts = [];
      replyImageParts = [];
      const avatarPart = await this.getReplyAvatarImagePart(replyMsg);
      if (avatarPart) imageParts = [avatarPart, ...messageImageParts];
    } else if (sourceLock === "sticker") {
      imageParts = [...replyImageParts.filter((part, idx) => idx < replyImageParts.length), ...messageImageParts];
    } else if (sourceLock === "image") {
      imageParts = [...replyImageParts, ...messageImageParts];
    }

    // 🆕 明确说“头像”时，优先取被回复用户头像，避免回复贴纸时误重绘贴纸
    if (wantsAvatar) {
      imageParts = [];
      replyImageParts = [];
      ...
      if (!avatarLoaded) {
        // 头像取不到时，不要直接中断；如果用户回复的是图片/贴纸，就退回到图片/贴纸重绘
        replyImageParts = originalReplyImageParts;
        imageParts = originalImageParts;
        if (imageParts.length === 0) {
          throw new UserError("无法获取被回复用户的头像，请确认对方有公开头像，或直接回复图片/贴纸重绘");
        }
      }
    }
    ...
    const hasPrompt = !!cleanedPromptInput;
    ...
    if (
      ...
      throw new UserError(
        `请先配置 API 并设置模型\n使用 ${prefixes[0]}aix config add <tag> <url> <key> [type] 和 ${prefixes[0]}aix model image <tag> <model-path>`,
      );
    }
    ...
    try {
      ...
      if (referenceCount > 0) {
        generationStatusMsg = await MessageSender.sendNew(
          msg,
          `🔍 正在识别第 1/${referenceCount} 张图片...`,
        );
        ...
        generationStatusMsg = await replaceStatusMessage(
          ...
          msg,
          optimizePrompt
            ? `✅ 已识别 ${referenceCount} 张图片，正在优化提示词...`
      ...
      if (optimizePrompt) {
        ...
        if (imageCount > 1) {
          ...
          for (let i = 0; i < imageCount; i++) {
            token.throwIfAborted();
            const explicitPrompt = normalizeSingleImagePrompt(originalPrompt, i, imageCount);
            ...
            optimizedPrompts.push(sanitizeOptimizedImagePrompt(optimized.text, basePrompt));
            if (i < imageCount - 1) await sleep(1200, token);
          }
      ...
      const imageCaption = imageCount > 1
        ? buildGeneratedPromptListText("优化后提示词", perImagePrompts.length > 0 ? perImagePrompts : Array.from({ length: imageCount }, (_, i) => normalizeSingleImagePrompt(prompt, i, imageCount)))
        : (optimizedPromptText || normalizedPromptForImage);
      ...
      if (imageParts.length > 0) {
        let inputImage = await resolveMergedImageParts(
          imageParts,
          this.httpClient,
          token,
        );
        ...
        for (let i = 0; i < imageCount; i++) {
          token.throwIfAborted();
          const singlePrompt = perImagePrompts[i] || normalizeSingleImagePrompt(prompt, i, imageCount);
          for (let attempt = 1; attempt <= 2; attempt++) {
            try {
              const batch = await this.aiService.editImage(singlePrompt, inputImage, token);
              images.push(...batch);
              break;
            } catch (error: any) {
              if (!isTransientImageError(error) || attempt >= 2) {
                failures.push(imageCount > 1 ? `第${i + 1}张: ${describeImageError(error)}` : describeImageError(error));
                break;
              }
              await sleep(3500, token);
            }
          }
          if (i < imageCount - 1) await sleep(3500, token);
        }
      } else {
        for (let i = 0; i < imageCount; i++) {
          token.throwIfAborted();
          ...
          for (let attempt = 1; attempt <= 2; attempt++) {
            try {
              const batch = await this.aiService.generateImage(singlePrompt, token);
              images.push(...batch);
              break;
            } catch (error: any) {
              if (!isTransientImageError(error) || attempt >= 2) {
      ...
      if (images.length === 0 && optimizePrompt && prompt !== originalPrompt) {
        if (imageParts.length > 0) {
          let fallbackInputImage = await resolveMergedImageParts(
            imageParts,
            this.httpClient,
            token,
          );
          if (!fallbackInputImage?.data) throw new Error("无法解析图片数据");
          if (fallbackInputImage.data && fallbackInputImage.mimeType !== "image/png") {
            try {
              const pngBuffer = await sharp(fallbackInputImage.data).png().toBuffer();
              fallbackInputImage = { data: pngBuffer, mimeType: "image/png" };
            } catch {}
          }
          for (let i = 0; i < imageCount; i++) {
            token.throwIfAborted();
            try {
              const singlePrompt = normalizeSingleImagePrompt(originalPrompt, i, imageCount);
              const batch = await this.aiService.editImage(singlePrompt, fallbackInputImage, token);
              images.push(...batch);
            } catch {}
            if (i < imageCount - 1) await sleep(3500, token);
          }
      ...
      if (images.length === 0) {
        if (wantsAvatar && imageParts.length > 0) {
          const avatarPrompt = `${prompt}

请根据我提供的头像特征生成表情包风格图片，保留人物/头像核心特征，但输出为新的独立表情包，不要直接返回空结果。`;
          const retryImages: AIImage[] = [];
          for (let i = 0; i < imageCount; i++) {
            token.throwIfAborted();
            const singlePrompt = normalizeSingleImagePrompt(avatarPrompt, i, imageCount);
            try {
              const batch = await this.aiService.generateImage(singlePrompt, token);
              retryImages.push(...batch);
            } catch {}
            if (i < imageCount - 1) await sleep(3500, token);
          }
      ...
      if (images.length === 0) {
        ...
        throw new Error(detail || "AI 回复为空");
      }
      imageGenerationDone = true;
      if (imageCount > 1) {
        await this.messageUtils.sendImages(msg, images, imageCaption, replyToId, token);
        if (generationStatusMsg) scheduleDeleteMessage(generationStatusMsg, 1);
        if (failures.length > 0) {
          await MessageSender.sendNew(msg, `⚠️ 已成功生成 ${images.length} 张，剩余张数失败。
<blockquote expandable>${htmlEscape(failures.join("\n"))}</blockquote>`, { parseMode: "html" }, replyToId);
        }
      } else {
        await this.messageUtils.sendImages(msg, images.slice(0, 1), imageCaption, replyToId, token);
        if (generationStatusMsg) scheduleDeleteMessage(generationStatusMsg, 1);
      }
      await deleteMessageOrGroup(msg);
    } finally {
...
class VideoFeature extends BaseFeatureHandler {
  ...
  async execute(
    ...
    // 视频提示词优化总开关：.aix video optimize on/off
    // on=先用聊天模型优化提示词再生成视频；off=直接用原提示词生成视频。
    if (subCommand === "optimize" && (args[2]?.toLowerCase() === "on" || args[2]?.toLowerCase() === "off") && args.length <= 3) {
      const enabled = args[2]?.toLowerCase() === "on";
      await configManager.updateConfig((cfg) => {
        cfg.videoPromptOptimize = enabled;
      });
      await this.editMessage(
        msg,
        enabled ? "✅ 视频提示词优化已开启" : "✅ 视频提示词优化已关闭",
      );
      return;
    }
    if (subCommand === "optimize" && ["short", "medium", "long"].includes(args[2]?.toLowerCase() || "")) {
      const mode = normalizePromptLengthMode(args[2]);
      await configManager.updateConfig((cfg) => {
        cfg.videoPromptLength = mode;
      });
      await this.editMessage(msg, `✅ 视频优化提示词长度已设为 ${mode}（${VIDEO_PROMPT_LENGTH_LABELS[mode]}）`);
      return;
    }
    ...
    let videoCount = Math.max(1, videoRequest.count || 1);
    ...
    if (forceAvatar) {
      replyImageParts = [];
      const avatarPart = await this.getReplyAvatarImagePart(replyMsg);
      if (avatarPart) replyImageParts = [avatarPart];
    }
    ...
    if (
      ...
      throw new UserError(
        `请先配置 API 并设置模型\n使用 ${prefixes[0]}aix config add <tag> <url> <key> [type] 和 ${prefixes[0]}aix model video <tag> <model-path>`,
      );
    }
    ...
    try {
      ...
      if (config.videoPromptOptimize !== false && finalPrompt.trim()) {
        ...
        if (videoCount > 1) {
          ...
          for (let i = 0; i < videoCount; i++) {
            token.throwIfAborted();
            const explicitPrompt = normalizeSingleImagePrompt(finalPrompt, i, videoCount);
            const basePrompt = `${explicitPrompt}\n\n第${i + 1}个视频必须和其他视频在动作、镜头或文案重点上明显不同，不能只做同义改写。`;
            const optimized = await runPromptOptimizationWithTimeout(
              this.aiService,
              buildVideoPromptOptimizationRequest(basePrompt, config.videoPromptLength),
              allImageParts,
              token,
              Math.max((config.timeout || 30), 20) * 1000,
            );
            ...
            optimizedPrompts.push(sanitizeOptimizedImagePrompt(optimized.text, basePrompt));
            if (i < videoCount - 1) await sleep(1200, token);
          }
          finalPrompt = optimizedPrompts[0] || finalPrompt;
          generationStatusMsg = await replaceStatusMessage(
            generationStatusMsg,
            msg,
            buildMultiPromptStatusText("视频提示词已分别优化，正在生成视频...", optimizedPrompts),
          );
          await sleep(10000, token);
          (msg as any).__aixVideoPromptList = optimizedPrompts;
        } else {
          const optimized = await runPromptOptimizationWithTimeout(
            this.aiService,
          ...
          (msg as any).__aixVideoPromptList = [finalPrompt];
        }
        generationStatusMsg = await replaceStatusMessage(
          generationStatusMsg,
          msg,
      ...
      const videoPromptList: string[] = Array.isArray((msg as any).__aixVideoPromptList)
        ? (msg as any).__aixVideoPromptList
        : [];
      const normalizedVideoPrompts = videoCount > 1
        ? (videoPromptList.length > 0 ? videoPromptList : Array.from({ length: videoCount }, (_, i) => normalizeSingleImagePrompt(finalPrompt, i, videoCount)))
        : [finalPrompt];
      const videos: AIVideo[] = [];
      for (let i = 0; i < normalizedVideoPrompts.length; i++) {
        token.throwIfAborted();
        const batch = await this.aiService.generateVideo(
        ...
        videos.push(...batch);
        if (i < normalizedVideoPrompts.length - 1) await sleep(3500, token);
      }
      if (videos.length === 0) throw new Error("AI 回复为空");
      const videoCaption = videoCount > 1
        ? buildGeneratedPromptListText("优化后提示词", normalizedVideoPrompts)
        : finalPrompt;
...
<b>💬 主要用法:</b>
• <code>${mainPrefix}aix input</code> - 向 AI 发起提问
• <code>${mainPrefix}aix search prompt</code> - 联网搜索并回答
• <code>${mainPrefix}aix image prompt</code> - 生图/回复改图/多图结合
• <code>${mainPrefix}aix image 2 prompt</code> - 生成2张图片
• <code>${mainPrefix}aix video prompt</code> - 文生/参考图生成视频
• <code>${mainPrefix}aix video first prompt</code> - 首帧生成视频
• <code>${mainPrefix}aix video firstlast prompt</code> - 首尾帧生成视频
• <code>a</code> - 用于识别回复目标头像
• 多图结合：一次发送多张图片，或回复图片后附带其他图片
• 发送图片时，命令和 prompt 必须写在图片下方的说明中
• 例如：<code>${mainPrefix}aix image 2 prompt</code> / <code>${mainPrefix}aix image a prompt</code> / <code>${mainPrefix}aix image a 2 prompt</code>
• 组合用法：回复目标消息并附带多张图片，在图片说明中输入 <code>${mainPrefix}aix image a 2 prompt</code>，可结合头像与多张图片生成2张

<b>⚙️ 配置:</b>
• <code>${mainPrefix}aix config list</code> - 查看配置
• <code>${mainPrefix}aix config add tag url key [type]</code> - 添加配置，未填 type 时按链接自动识别
• <code>${mainPrefix}aix config del tag</code> - 删除配置
• <code>${mainPrefix}aix config type tag openai-compatible|openai|gemini|doubao|moonshot|local-cliproxy</code> - 修改配置类型
• <code>${mainPrefix}aix config stream tag on|off</code> - 流式开关
• <code>${mainPrefix}aix config responses tag on|off</code> - Responses API 开关

<b>🤖 模型:</b>
• <code>${mainPrefix}aix model</code> - 查看当前模型
• <code>${mainPrefix}aix model chat tag model</code> - 设置对话模型
• <code>${mainPrefix}aix model search tag model</code> - 设置搜索模型
• <code>${mainPrefix}aix model image tag model</code> - 设置图片模型
• <code>${mainPrefix}aix model video tag model</code> - 设置视频模型
• <code>${mainPrefix}aix model preset add 配置名 提供商 模型</code> - 保存命名模型预设
• <code>${mainPrefix}aix model preset list</code> - 查看模型预设
• <code>${mainPrefix}aix model preset del 配置名</code> - 删除模型预设
• <code>${mainPrefix}aix model chat 配置名</code> - 用预设一键切换（search/image/video 同理）
...
• <code>${mainPrefix}aix video preview on|off</code> - 视频预览
• <code>${mainPrefix}aix video audio on|off</code> - 视频音频
• <code>${mainPrefix}aix video duration 秒数</code> - 视频时长
• <code>${mainPrefix}aix collapse on|off</code> - 折叠输出
• <code>${mainPrefix}aix telegraph on|off</code> - 长文 Telegraph
• <code>${mainPrefix}aix timeout 秒数</code> - 设置超时
• <code>${mainPrefix}aix stop</code> - 取消正在执行的 AIX 任务`;
  ...
  > = {
    aix: async (msg: Api.Message, trigger?: Api.Message) => {
      try {
        ...
        if (sub === "stop") {
          this.questionFeature.cancelCurrentOperation("__AIX_SILENT_CANCEL__");
          this.aiService.cancelAllOperations("__AIX_SILENT_CANCEL__");
          await MessageSender.sendOrEdit(trigger || msg, "🚫 操作已取消");
          return;
        }
